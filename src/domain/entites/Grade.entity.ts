...
import { GradeModel, GradeSchema } from '../../database';
...
export class GradeEntity {
  ...
  toEntity(v: GradeSchema) {
    return v
      ? this.setId(v._id)
          .setAppliedTaskId(v.appliedTaskId)
          .setUserId(v.userId)
          .setOriginalPoint(v.originalPoint)
          .setPenaltyPoint(v.penaltyPoint)
          .setFinalPoint(v.finalPoint)
          .setStatus(v.status)
  ...
  toSchema(): GradeSchema {
    return this
      ? {
          _id: this.getId(),
          appliedTaskId: this.getAppliedTaskId(),
          userId: this.getUserId(),
          originalPoint: this.getOriginalPoint(),
          penaltyPoint: this.getPenaltyPoint(),
          finalPoint: this.getFinalPoint(),
  ...
  async create() {
    const grade = await GradeModel.create(this.toSchema());
    return this.toEntity(grade);
  }
}
